...
def cw_attack(dataset_name, nn_type, save_dir, eps, gpu_list,
    ...
    if targeted:
        ...
        #preds_op = tf.nn.softmax(model.logits_test)
        #preds = []
        #for i in xrange(n_batches):
        #    batch = X_test[i*batch_size:(i+1)*batch_size]
        #    preds_ = run_ops_test(model, preds_op, {model.x: batch})
        #    preds.extend(preds_)
        #preds = np.asarray(preds)
        #preds = np.argsort(preds, axis=1)[:, ::-1]
        #top2 = preds[:, 1]
        #one_hot = np.zeros((source_samples, nb_classes))
        #one_hot[np.arange(source_samples), top2] = 1

        targets = []
        for i in xrange(source_samples):
            while True:
                t = random.randint(0, 9)
                if t != np.argmax(Y_test[i]): break
            targets.append(t)
        targets = np.asarray(targets)
        one_hot = np.zeros((source_samples, nb_classes))
        one_hot[np.arange(source_samples), targets] = 1
        adv_ys = one_hot
...
if __name__ == '__main__':
    ...
    #flags.DEFINE_boolean('targeted', True, 'Run in targeted mode?')
    flags.DEFINE_boolean('targeted', False, 'Run in targeted mode?')
