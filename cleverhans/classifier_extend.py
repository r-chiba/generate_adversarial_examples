...
def fprop(self, x):
    if 'vgg' in str(self.__class__):
        codes = self.model(x, 1.0, training=False, reuse=True)
    else:
        codes = self.model(x, training=False, reuse=True)
...
def classifier_extend(clf):
    clf.__class__.__call__ = call
    clf.__class__.get_layer_names = get_layer_names
    clf.__class__.fprop = fprop
    clf.__class__.get_layer = get_layer
    clf.__class__.get_logits = get_logits
    clf.__class__.get_probs = get_probs
    return clf
